...
const Table = ({ products, pageInfo }: ITableProps) => {
  ...
  const handleProductUpdated = useCallback(
    (updatedProduct: UpdateProductResponse["product"]) => {
      if (!updatedProduct) return;

      setProductList((prevProducts) =>
        prevProducts.map((product) =>
          product.id === updatedProduct.shopifyId
            ? { ...product, disabled: updatedProduct.disabled }
            : product
        )
      );
    },
        ...
        <s-table-body>
          {productList.map((product) => (
            <s-table-row key={product.id}>
              <s-table-cell>
                <s-thumbnail src={product.image ?? ""} alt={product.alt ?? ""} />
              </s-table-cell>

              <s-table-cell>{product.title}</s-table-cell>

              <s-table-cell>
                <ProductButton product={product} onUpdated={handleProductUpdated} />
              </s-table-cell>
            </s-table-row>
          ))}
        </s-table-body>
